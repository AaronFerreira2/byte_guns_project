	...
	if is_multiplayer_authority():
		...
		if mouse_position.x < global_position.x:
			if Global.second_color == true:
				$AnimatedSprite2D2.flip_h = true  # Vira o sprite para a esquerda
			else:
				$AnimatedSprite2D.flip_h = true
		else:
			if Global.second_color == true:
				$AnimatedSprite2D2.flip_h = false  # Vira o sprite para a direita
			else:
...
func _physics_process(_delta: float) -> void:
	...
	if !grounded:
		if mouse_position.y > middle_y + middle:
			if Global.second_color == true:
				$AnimatedSprite2D2.play("jump down2")
			else: # Mouse está acima do meio-termo (subindo)
				$AnimatedSprite2D.play("jump down")
		elif mouse_position.y < middle_y - middle:
			if Global.second_color == true:
				$AnimatedSprite2D2.play("jump up2")
			else: # Mouse está abaixo do meio-termo (descendo)
