...
func _on_area_entered(area: Area2D) -> void:
	if area.name == "hurtbox":
		...
		if multiplayer.get_unique_id() == playerqueentrou.name.to_int() and Global.bullets_shotgun < Player.MAX_BULLETS:
			rpc("_update_visibility", false)
			Global.bullets_shotgun = Player.MAX_BULLETS
		if Global.bullets_shotgun == Player.MAX_BULLETS:
			Global.bullets_shotgun += 0  # Notifica todos os clientes que a munição foi coletada
