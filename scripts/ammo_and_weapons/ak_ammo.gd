...
func _on_area_entered(area: Area2D) -> void:
	if area.name == "hurtbox":
		...
		if multiplayer.get_unique_id() == playerqueentrou.name.to_int() and Global.bullets_ak < Player.MAX_BULLETS:
			rpc("_update_visibility", false)
			Global.bullets_ak = Player.MAX_BULLETS
		elif multiplayer.get_unique_id() == playerqueentrou.name.to_int() and Global.ak_unlocked == false:
			Global.ak_unlocked = true
			rpc("_update_visibility", false)
		if Global.bullets_ak == Player.MAX_BULLETS:
			Global.bullets_ak += 0  # Notifica todos os clientes que a munição foi coletada
