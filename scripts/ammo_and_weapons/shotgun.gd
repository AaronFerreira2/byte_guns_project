extends Node2D
@onready var shotgun = $"."  # Referência à arma
...
func _process(delta: float) -> void:
	...
	# Mostra a arma se o jogador estiver usando a shotgun
		if Global.using_shotgun == true:
			...
			if Global.bullets_shotgun <= 0:
				can_still_shoot = false
		elif Global.using_shotgun == false:
			shotgun.hide()
			can_still_shoot = false
	...
	# Alterna entre usar e não usar a shotgun
		if Input.is_action_just_pressed("shotgun"):
			print("Shotgun")
			Global.using_shotgun = !Global.using_shotgun
			Global.using_ak = false
			Global.using_revolver = false
	...
	# Verifica se o botão de disparo foi pressionado e se o timer terminou
		if Input.is_action_just_pressed("fire") and can_shoot and can_still_shoot:
			...
			# Inicia o timer e bloqueia novos disparos até o timeout
				...
				player.current_bullets = Global.bullets_shotgun

		if Global.bullets_shotgun <= 0:
			can_still_shoot = false
		elif Global.bullets_shotgun == 30:
			can_still_shoot = true
...
@rpc("call_remote")
func bullets():
	Global.bullets_shotgun -= 1
	print(Global.bullets_shotgun)
