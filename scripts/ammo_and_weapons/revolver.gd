extends Node2D
@onready var revolver: Node2D = $"."

const BULLET = preload("res://scenes/bullets/bullet3.tscn")
...
var rotation_speed := 90.0  # Velocidade de rotaçãox (graus por segundo)
...
func _process(delta: float) -> void:
	...
	# Mostra a arma se o jogador estiver usando a shotgun
		if Global.using_revolver == true:
			revolver.show()
			can_still_shoot = true
			Global.using_shotgun = false
			Global.using_ak = false
			if Global.bullets_revolver <= 0:
				can_still_shoot = false
		elif Global.using_revolver == false:
			revolver.hide()
			can_still_shoot = false
		if Global.revolver_unlocked == true and Global.first_using_revolver == true:
			Global.first_using_revolver = false
			Global.using_revolver = true
			Global.using_ak = false
			Global.using_shotgun = false
	# Alterna entre usar e não usar a shotgun
		if Input.is_action_just_pressed("revolver") and Global.revolver_unlocked:
			print("Revolver")
			Global.using_revolver = !Global.using_revolver
			Global.using_shotgun = false
			Global.using_ak = false
		if player == null:
	...
	# Verifica se o botão de disparo foi pressionado e se o timer terminou
		if Input.is_action_pressed("fire") and can_shoot and can_still_shoot:
			...
			# Inicia o timer e bloqueia novos disparos até o timeout
				...
				player.current_bullets = Global.bullets_revolver

		if Global.bullets_revolver <= 0:
			can_still_shoot = false
		elif Global.bullets_revolver == 6:
			can_still_shoot = true
...
@rpc("call_remote")
func bullets():
	Global.bullets_revolver -= 1
	print(Global.bullets_revolver)
